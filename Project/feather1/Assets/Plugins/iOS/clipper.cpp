...
bool RangeTest(const IntPoint& Pt, bool& useFullRange)
{
  if (useFullRange)
  {
    ...
    if (Pt.X > hiRange || Pt.Y > hiRange || -Pt.X > hiRange || -Pt.Y > hiRange)
    {
      fprintf( stderr, "Coordinate outside allowed range\n" );
      return false;
    }
  }
...
bool ClipperBase::AddPath(const Path &pg, PolyType PolyTyp, bool Closed)
{
  ...
  if (!Closed && PolyTyp == ptClip)
  {
    fprintf( stderr, "AddPath: Open paths must be subject.\n" );
    return false;
  }
#else
  //if (!Closed)
  ...
  if (!Closed)
  {
    fprintf( stderr, "AddPath: Open paths have been disabled.\n" );
    return false;
  }
#endif
  ...
  {
    edges[1].Curr = pg[1];
    InsideRange = RangeTest(pg[0], m_UseFullRange);
    if (!InsideRange)
    {
      delete [] edges;
      return false;
    ...
    InsideRange = RangeTest(pg[highI], m_UseFullRange);
    if (!InsideRange)
    {
      delete [] edges;
      return false;
    ...
    {
      InsideRange = RangeTest(pg[i], m_UseFullRange);
      if (!InsideRange)
      {
        delete [] edges;
        return false;
    ...
    PolyFillType subjFillType, PolyFillType clipFillType)
{
  ...
  if (m_HasOpenPaths)
  {
    fprintf( stderr, "Error: PolyTree struct is need for open path clipping.\n" );
    return false;
  }
...
bool Clipper::ExecuteInternal()
{
  ...
  //try {
    ...
    do {
      succeeded = InsertLocalMinimaIntoAEL(botY);
      if (!succeeded)
        break;

      ClearGhostJoins();

      succeeded = ProcessHorizontals(false);
      if (!succeeded)
        break;
      ...
      succeeded = ProcessEdgesAtTopOfScanbeam(topY);
      if (!succeeded)
        break;
...
bool Clipper::InsertLocalMinimaIntoAEL(const cInt botY)
{
    ...
    if(lb->NextInAEL != rb)
    {
      ...
      if (e)
      {
        while( e != rb )
        {
          ...
          bool res = IntersectEdges(rb , e , lb->Curr); //order important here
          if (res == false)
            return res;
...
bool Clipper::IntersectEdges(TEdge *e1, TEdge *e2,
     const IntPoint &Pt, bool protect)
{
  ...
  if (e1->WindDelta == 0 || e2->WindDelta == 0)
  {
    ...
    if (e1stops)
      ...
      {
        fprintf( stderr, "Error intersecting polylines\n" );
        return false;
      }
    if (e2stops) 
...
bool Clipper::ProcessHorizontals(bool IsTopOfScanbeam)
{
  TEdge* horzEdge = m_SortedEdges;
  while(horzEdge)
  {
    DeleteFromSEL(horzEdge);
    bool res = ProcessHorizontal(horzEdge, IsTopOfScanbeam);
    if (res == false)
      return res;
...
bool Clipper::ProcessHorizontal(TEdge *horzEdge, bool isTopOfScanbeam)
{
  Direction dir;
  ...
  for (;;)
  {
    ...
    while(e)
    {
        ...
        (dir == dRightToLeft && e->Curr.X >= horzLeft))
      {
        ...
        if(e == eMaxPair && IsLastHorz)
        {
          if (dir == dLeftToRight)
          {
            bool res = IntersectEdges(horzEdge, e, e->Top);
            if (res == false)
              return res;
          }
          else
          {
            bool res = IntersectEdges(e, horzEdge, e->Top);
            if (res == false)
              return res;
          }
          ...
          {
            fprintf( stderr, "ProcessHorizontal error\n" );
            return false;
          }
          return true;
        }
        else if(dir == dLeftToRight)
        {
          ...
          bool res = IntersectEdges(horzEdge, e, Pt, true);
          if (res == false)
            return res;
        }
        else
        {
          IntPoint Pt = IntPoint(e->Curr.X, horzEdge->Curr.Y);
          bool res = IntersectEdges( e, horzEdge, Pt, true);
          if (res == false)
            return res;
        }
    ...
    {
      bool res = UpdateEdgeIntoAEL(horzEdge);
      if (res == false)
        return res;
  ...
  if(horzEdge->NextInLML)
  {
    if(horzEdge->OutIdx >= 0)
    {
      ...
      if (horzEdge->WindDelta == 0) return true;
  ...
  else if (eMaxPair)
  {
    if (eMaxPair->OutIdx >= 0)
    {
      if (dir == dLeftToRight)
      {
        bool res = IntersectEdges(horzEdge, eMaxPair, horzEdge->Top);
        if (res == false)
          return res;
      }
      else
      {
        bool res = IntersectEdges(eMaxPair, horzEdge, horzEdge->Top);
        if (res == false)
          return res;
      }
      if (eMaxPair->OutIdx >= 0)
...
bool Clipper::UpdateEdgeIntoAEL(TEdge *&e)
{
  //if( !e->NextInLML ) throw
  //  clipperException("UpdateEdgeIntoAEL: invalid call");
  if( !e->NextInLML )
  {
    fprintf( stderr, "UpdateEdgeIntoAEL: invalid call\n" );
    return false;
  }
...
bool Clipper::ProcessIntersections(const cInt botY, const cInt topY)
{
  ...
  //try {
    bool res = BuildIntersectList(botY, topY);
    if (res == false)
    {
      m_SortedEdges = 0;
      DisposeIntersectNodes();

      fprintf( stderr, "ProcessIntersections error");
      return false;
    }
...
bool Clipper::BuildIntersectList(const cInt botY, const cInt topY)
{
  if ( !m_ActiveEdges ) return true;
...
bool Clipper::ProcessIntersectList()
{
  for (size_t i = 0; i < m_IntersectList.size(); ++i)
  {
    ...
    {
      bool res = IntersectEdges( iNode->Edge1, iNode->Edge2, iNode->Pt, true);
      if (res == false)
        return res;
...
bool Clipper::DoMaxima(TEdge *e)
{
  ...
  {
    bool res = IntersectEdges(e, eNext, e->Top, true);
    if (res == false)
      return res;
  ...
  {
    bool res = IntersectEdges( e, eMaxPair, e->Top);
    if (res == false)
      return res;
  }
#ifdef use_lines
...
bool Clipper::ProcessEdgesAtTopOfScanbeam(const cInt topY)
{
  ...
  bool res = ProcessHorizontals(true);
  if (res == false)
    return false;
